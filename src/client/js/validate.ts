...
var defaults = {
    ...
    callback: function (errors: ValidatorError[]) {
        for (let i = 0; i < errors.length; i++) {
            for (let j = 0; j < errors[i].messages.length; j++) {
                ...
                errorTextElement.innerHTML = errors[i].messages[j];
...
export class FormValidator {
    ...
    constructor(formNameOrNode, fields: ValidateField[], callback) {
    ...
    clearErrors() {
        let errorElements = this.form.querySelectorAll('.' + defaults.errorClassName);
