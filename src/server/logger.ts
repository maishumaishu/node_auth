...
import { tableNames, execute } from './database';
import * as http from 'http';

export function logUserRequest(request: express.Request) {
    return execute(settings.conn.log, tableNames.RequestLog, function (table) {
...
export function logRedirectRequest(request: http.ClientRequest) {
    return execute(settings.conn.log, tableNames.RedirectLog, function (table) {
